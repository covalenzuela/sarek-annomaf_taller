...
# - Descarga FASTQ idempotente con verificación.
# - Genera samplesheet válido (Sarek 3.x).
# - Un SOLO -params-file (params.yaml) que incluye run + anotación (snpeff/vep).
...
#   TOOLS="haplotypecaller"          # sobre-escribe defaults por modo (en minúsculas)
...
verify_fastq() {
  ...
  if ! gzip -t "$f" 2>/dev/null; then
    echo "[ERR] gzip corrupto: $f"; return 1
  fi
  local head4 L1 L3
  ...
  if [[ "${L1:0:1}" != "@" || "${L3:0:1}" != "+" ]]; then
    echo "[ERR] Estructura FASTQ inválida: $f"; return 1
  fi
...
_fetch() {
  ...
  if command -v aria2c >/dev/null 2>&1; then
    aria2c -x 8 -s 8 -c --retry-wait=5 --max-tries=5 \
...
dl() {
  ...
  while (( attempt <= MAX_RETRIES )); do
    ...
    echo "[WARN] Descarga inválida; reintentando: $out"
    rm -f "$out"
    attempt=$((attempt+1))
  done
...
# ---------- Construir params.yaml (ÚNICO) ----------
PARAMS_FILE="$WORKDIR/params.yaml"

# Defaults por modo (si no se sobre-escriben con TOOLS/ALIGNER)
ALIGNER="${ALIGNER:-bwa-mem2}"
if [[ "$MODE" =~ ^germ(inal)?$ ]]; then
  TOOLS_LIST="${TOOLS:-haplotypecaller}"           # germinal
else
  TOOLS_LIST="${TOOLS:-mutect2,strelka}"           # somático
fi

# Asegurar minúsculas y sin espacios
TOOLS_LOWER="$(echo "$TOOLS_LIST" | tr '[:upper:]' '[:lower:]' | sed 's/[[:space:]]//g')"
